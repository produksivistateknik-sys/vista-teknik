...
// ─── STEP 1: Pindah useState sidebarCollapsed ke posisi benar ───
const tabStateLine = lines.findIndex(l => l.includes('const [tab,setTab]=useState("dashboard")'));
console.log('tab state di baris:', tabStateLine+1);

// Hapus sidebarCollapsed dari mana pun
const cleanLines = lines.filter(l => !l.includes('const [sidebarCollapsed,setSidebarCollapsed]=useState(false)'));

// Tambah setelah tab state
const tabIdx = cleanLines.findIndex(l => l.includes('const [tab,setTab]=useState("dashboard")'));
cleanLines.splice(tabIdx+1, 0, '  const [sidebarCollapsed,setSidebarCollapsed]=useState(false);');
console.log('✅ Step 1: useState dipindah ke baris', tabIdx+2);

// ─── STEP 2: Replace layout lama dengan ERP layout ───
const joined = cleanLines.join('\n');

const startMarker = 'const monitorTabs=[';
const endMarker = 'tab==="kendala"&&<KendalaInbox';

const startIdx = joined.indexOf(startMarker);
const endIdx = joined.indexOf(endMarker);
const endOfLine = joined.indexOf('\n', endIdx);

console.log('Layout lama start:', startIdx, 'end:', endOfLine);
...
const result = joined.slice(0, startIdx) + newLayout;
fs.writeFileSync('src/App.tsx', result, 'utf8');
console.log('✅ Full patch selesai!');
console.log('Total chars:', result.length);
console.log('erp-wrap:', result.includes('erp-wrap'));
console.log('sidebarCollapsed before alerts:', result.indexOf('sidebarCollapsed') < result.indexOf('const alerts'));
console.log('Last 50 chars:', JSON.stringify(result.slice(-50)));
