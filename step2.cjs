...
const startLine = lines.findIndex(l => l.includes('const monitorTabs=['));
const endLine = lines.findIndex(l => l.includes('tab==="kendala"&&<KendalaInbox'));
console.log('Start:', startLine, 'End:', endLine);

const before = lines.slice(0, startLine);
const after = lines.slice(endLine + 1);

const newBlock = `  const alerts=woData.filter(w=>woOverall(w)<100&&(isDelayed(w.target)||isUrgent(w.target))).length;
...
console.log('✅ Layout ERP berhasil!');
console.log('Total baris:', newLines.length);
console.log('erp-wrap:', newLines.join('\n').includes('erp-wrap'));
console.log('erp-sidebar:', newLines.join('\n').includes('erp-sidebar'));