...
function RencanaHarian({rawData,woData,renhar,setRenhar,pekerja,createRenhar,updateRenhar,removeRenhar}){
  ...
  // Ambil semua tugas dari rawData untuk tanggal terpilih
  ...
  const byProses=useMemo(()=>{
    ...
    filteredTasks.forEach(t=>{
      if(!map[t.proses])map[t.proses]=[];
      map[t.proses].push(t);
    });
    return map;
  },[filteredTasks]);

  const prosesList=selProses==="ALL"?Object.keys(byProses):Object.keys(byProses);

  // Hitung berapa tugas per hari untuk calendar strip
  const taskCountByDay=useMemo(()=>{
    const map={};
    days.forEach(d=>{
      let count=0;
      rawData.forEach(row=>{const e=row.schedule[d]||[];count+=e.length;});
      map[d]=count;
    });
    return map;
  },[days,rawData]);
  ...
  return(
    <div className="fi">
          ...
          {days.map(d=>{
            ...
            return(
              <button key={d} onClick={()=>setSelDate(d)}
                ...
                <div style={{fontSize:10,fontWeight:600,color:isSel?"#fff":isToday?"#2563eb":"#94a3b8",marginBottom:2}}>
                  {getDayLabel(d).split(" ")[0]}
                </div>
                <div style={{fontSize:14,fontWeight:800,color:isSel?"#fff":isToday?"#1d4ed8":"#1e293b"}}>
                  {getDayLabel(d).split(" ")[1]||d.slice(8)}
                </div>
                {cnt>0&&(
                  <div style={{marginTop:4,background:isSel?"#ffffff33":"#2563eb",borderRadius:20,
                    padding:"1px 6px",fontSize:9,fontWeight:700,color:isSel?"#fff":"#fff",display:"inline-block"}}>
                    {cnt}
                  </div>
                )}
              </button>
        ...
        <div style={{marginLeft:"auto",display:"flex",gap:8,flexWrap:"wrap",alignItems:"center"}}>
          {filteredTasks.length>0&&(
            <span style={{fontSize:12,color:"#64748b"}}>
              {distCount}/{filteredTasks.length} terdistribusi
            </span>
          )}
      ...
      <div style={{display:"flex",gap:5,marginBottom:14,flexWrap:"wrap"}}>
        <button onClick={()=>setSelProses("ALL")}
          style={{padding:"4px 14px",borderRadius:20,border:`1.5px solid ${selProses==="ALL"?"#1d4ed8":"#e2e8f0"}`,
            background:selProses==="ALL"?"#1d4ed8":"#fff",color:selProses==="ALL"?"#fff":"#64748b",
            cursor:"pointer",fontSize:11,fontWeight:700}}>
          Semua ({allTasks.length})
        </button>
        {ALL_PROSES.filter(pr=>allTasks.some(t=>t.proses===pr)).map(pr=>{
          const pc=PROSES_COLOR[pr]||"#64748b";
          const cnt=allTasks.filter(t=>t.proses===pr).length;
          const isSel=selProses===pr;
          return(
            <button key={pr} onClick={()=>setSelProses(isSel?"ALL":pr)}
              style={{padding:"4px 14px",borderRadius:20,border:`1.5px solid ${isSel?pc:"#e2e8f0"}`,
                background:isSel?pc+"18":"#fff",color:isSel?pc:"#64748b",
                cursor:"pointer",fontSize:11,fontWeight:700}}>
              {pr} ({cnt})
            </button>
          );
      ...
      {prosesList.map(proses=>{
        ...
        const dc=DIVISI_CONFIG[divisiKey];
        const distTasks=tasks.filter(isDist).length;

        const thS={background:"#1e3a8a",color:"#fff",padding:"9px 10px",fontWeight:700,
          fontSize:11,whiteSpace:"nowrap",textAlign:"left",position:"sticky",top:0,
          borderRight:"1px solid #ffffff18"};

        return(
          <div key={proses} style={{marginBottom:20}}>
            {/* Section Header */}
            <div style={{background:pc,borderRadius:"10px 10px 0 0",padding:"10px 16px",
              display:"flex",alignItems:"center",justifyContent:"space-between"}}>
              <div style={{display:"flex",alignItems:"center",gap:10}}>
                <span style={{fontWeight:900,fontSize:15,color:"#fff"}}>{proses}</span>
                {dc&&<span style={{background:"#ffffff25",color:"#fff",borderRadius:20,
                  padding:"2px 10px",fontSize:11,fontWeight:700}}>{dc.icon} {dc.label}</span>}
                <span style={{background:"#ffffff25",color:"#fff",borderRadius:20,
                  padding:"2px 10px",fontSize:11,fontWeight:700}}>{tasks.length} tugas</span>
              </div>
              <span style={{fontSize:11,color:"#ffffff99",fontWeight:600}}>
                {distTasks}/{tasks.length} terdistribusi
              </span>
            </div>
            ...
            <div style={{overflowX:"auto",border:"1px solid #e2e8f0",borderTop:"none",borderRadius:"0 0 10px 10px"}}>
              <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
                ...
                <tbody>
                  {tasks.map((t,i)=>{
                    const dist=isDist(t);
                    const rh=renhar.find(r=>r.rawId===t.rawId&&r.wp===t.wp&&r.tanggal===t.tanggal);
                    ...
                    const td={padding:"8px 10px",borderBottom:"1px solid #f1f5f9",
                      borderRight:"1px solid #f1f5f9",background:dist?"#f0fdf4":rBg,
                    ...
                    return(
                      <tr key={i}>
                        ...
                        <td style={{...td,textAlign:"center"}}>
                          <span style={{background:wc,color:"#fff",borderRadius:6,
                            padding:"2px 8px",fontSize:11,fontWeight:700}}>{t.wp}</span>
                        </td>
                        <td style={{...td,textAlign:"center"}}>
                          <span style={{background:priColor+"18",color:priColor,border:`1px solid ${priColor}33`,
                            borderRadius:20,padding:"2px 9px",fontSize:10,fontWeight:700}}>{t.prioritas}</span>
                        </td>
                        <td style={{...td}}>
                            ...
                            {t.komponen.map(k=>{
                              ...
                              return(
                                <span key={k} style={{background:"#f1f5f9",borderRadius:4,
                                  padding:"2px 7px",fontSize:10,color:"#475569",fontWeight:600}}>
                                  {item?.nama||k}
                                </span>
                              );
                            })}
          ...
          <Modal title={(assignModal.existing?"Edit":"Distribusi")+" — "+task.proses} onClose={()=>{setAssignModal(null);setSelPekerja([]);}} width={460}>
            ...
            {pekerjaDivisi.length===0?(
              <div style={{padding:"16px",background:"#f8fafc",borderRadius:8,fontSize:12,color:"#94a3b8",textAlign:"center"}}>
                Belum ada pekerja di divisi ini. Tambahkan di Master Pekerja.
              </div>
            ):(
                  ...
                  return(
                    <div key={p.id} onClick={()=>setSelPekerja(prev=>isSel?prev.filter(id=>id!==p.id):[...prev,p.id])}
                      style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",borderRadius:10,
                        cursor:"pointer",border:`1.5px solid ${isSel?(dc?.color||"#2563eb"):"#e2e8f0"}`,
                        background:isSel?(dc?.bg||"#eff6ff"):"#f8fafc",transition:"all .15s"}}>
            ...
            <div style={{display:"flex",gap:10,justifyContent:"flex-end"}}>
              ...
              <Btn color="#1d4ed8" onClick={confirmDistribute}>
                {assignModal.existing?"Simpan":"Distribusi"}
              </Btn>
            </div>
          </Modal>
        );
...
function RawSchedule({woData,rawData,setRawData,renhar,setRenhar,pekerja,createRaw,updateRaw,removeRaw,createRenhar,updateRenhar,removeRenhar}){
  ...
  const [filterProses,setFilterProses]=useState("ALL"); // poin 5
  const [filterProyek,setFilterProyek]=useState("ALL"); // poin 1
  const [filterPanel,setFilterPanel]=useState("ALL");
  const [expandedTasks,setExpandedTasks]=useState({}); // track which task cards are expanded

  // Status per komponen individual: belum_mulai | on_progress | finish
  ...
  // Poin 1&2: status sinkronisasi berdasarkan progress aktual
  // Status: belum_mulai | on_progress | finish
  const getTaskStatus=(row,date,wp,komponen)=>{
    const panelData=woData.flatMap(w=>w.panels).find(p=>p.id===row.panelId);
    if(!panelData)return"belum_mulai";
    // Cek HANYA proses spesifik di row ini (bukan seluruh divisi)
    const proses=row.proses;
    // Cek apakah semua komponen di WP ini sudah 100% untuk proses tsb
  ...
  // Poin 3: cek apakah WP sudah 100% selesai semua komponen
    ...
    <div className="fi">
      ...
      <div style={{display:"flex",gap:8,marginBottom:12,flexWrap:"wrap",alignItems:"center",
        ...
        <select value={filterPanel} onChange={e=>setFilterPanel(e.target.value)}
          style={{padding:"4px 10px",borderRadius:8,border:"1.5px solid #e2e8f0",
            background:"#f8fafc",fontSize:11,fontWeight:600,color:"#475569",cursor:"pointer",maxWidth:260}}>
          <option value="ALL">Semua Panel</option>
          {[...new Set(rawData.filter(r=>filterProyek==="ALL"||r.proyek===filterProyek).map(r=>r.panel))].map(p=>(
      ...
      <div style={{display:"flex",gap:6,marginBottom:12,flexWrap:"wrap",alignItems:"center"}}>
        {/* Poin 5: filter proses */}
        <div style={{display:"flex",gap:5,flexWrap:"wrap",alignItems:"center",marginRight:8}}>
          <span style={{fontSize:11,color:"#64748b",fontWeight:600}}>Filter Proses:</span>
          <button onClick={()=>setFilterProses("ALL")}
            style={{padding:"3px 12px",borderRadius:20,border:`1.5px solid ${filterProses==="ALL"?"#1d4ed8":"#e2e8f0"}`,
              background:filterProses==="ALL"?"#1d4ed8":"#fff",color:filterProses==="ALL"?"#fff":"#64748b",
              cursor:"pointer",fontSize:11,fontWeight:700}}>Semua</button>
          {ALL_PROSES.map(pr=>{
            const pc=PROSES_COLOR[pr]||"#64748b";
            const isSel=filterProses===pr;
            return(
              <button key={pr} onClick={()=>setFilterProses(isSel?"ALL":pr)}
                style={{padding:"3px 12px",borderRadius:20,border:`1.5px solid ${isSel?pc:"#e2e8f0"}`,
                  background:isSel?pc+"18":"#fff",color:isSel?pc:"#64748b",
                  cursor:"pointer",fontSize:11,fontWeight:700}}>{pr}</button>
            );
          })}
        </div>
      </div>
      <div style={{display:"flex",gap:6,marginBottom:12,flexWrap:"wrap",alignItems:"center"}}>
        {WP_LIST.map(wp=>(
          <span key={wp} style={{display:"flex",alignItems:"center",gap:4,fontSize:11,fontWeight:700,
            color:WP_COLOR[wp],background:WP_COLOR[wp]+"18",border:`1px solid ${WP_COLOR[wp]}33`,borderRadius:20,padding:"2px 10px"}}>
            <span style={{width:7,height:7,borderRadius:"50%",background:WP_COLOR[wp],display:"inline-block"}}/>{wp}
          </span>
        ))}
            ...
            <tr>
              ...
              {days.map(d=>(
                <th key={d} onClick={()=>setSelDate(d===selDate?null:d)}
                  style={{...thS,minWidth:120,cursor:"pointer",
              ...
              return(
                <tr key={row.id}>
                  ...
                  <td style={{...td,position:"sticky",left:490,zIndex:2,textAlign:"center"}}>
                    <select value={row.prioritas||"Sedang"} onChange={e=>updatePrioritasPanel(row.panelId,e.target.value)}
                      style={{padding:"2px 6px",borderRadius:6,border:`1.5px solid ${priColor}`,
                        background:priColor+"18",color:priColor,fontSize:10,fontWeight:700,cursor:"pointer"}}>
                  ...
                  {days.map(d=>{
                    ...
                    return(
                      <td key={d} style={{...td,textAlign:"center",padding:"4px",
                        background:isOver?"#eff6ff":isSelDate&&entries.length?"#f0f9ff":rBg,
                        ...
                        {entries.length>0?(
                            ...
                            {entries.map(e=>{
                              ...
                              return(
                                <div key={e.wp} style={{...statusStyle,color:"#fff",borderRadius:4,padding:"2px 6px",fontSize:10,fontWeight:700,display:"flex",alignItems:"center",gap:3}}>
                                  ...
                                  {e.wp}<span style={{fontSize:9,opacity:.8,marginLeft:2}}>({e.komponen.length})</span>
                                </div>
                              );
                            })}
                          </div>
                          ...
                          <div onClick={()=>openCellModal(row.id,d)}
                            style={{width:"100%",height:32,borderRadius:6,cursor:"pointer",border:"1px dashed #e2e8f0",
                              display:"flex",alignItems:"center",justifyContent:"center",color:"#e2e8f0",fontSize:16,transition:"all .15s"}}
              ...
              <div key={i} style={{padding:"10px 14px",borderRadius:10,marginBottom:8,background:"#fff",border:`1px solid ${statusColor}30`}}>
                ...
                <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:8,flexWrap:"wrap"}}>
                  <div style={{display:"flex",gap:4,flexWrap:"wrap",flex:1}}>
                    {t.komponen.map(k=>{
                      const item=cfg2?.wps.flatMap(w=>w.items).find(it=>it.kode===k);
                      return <span key={k} style={{background:"#f1f5f9",borderRadius:4,padding:"2px 8px",fontSize:10,color:"#475569",fontWeight:600}}>{item?.nama||k}</span>;
                    })}
                  </div>
                ...
                {isExpanded&&(
                  <div style={{marginTop:10,paddingTop:10,borderTop:"1px dashed #e2e8f0",display:"flex",flexDirection:"column",gap:8}}>
                    {statusGroups.filter(g=>grouped[g.key].length>0).map(g=>(
                      <div key={g.key} style={{background:g.bg,border:`1px solid ${g.border}`,
                        borderRadius:8,padding:"8px 12px"}}>
                        <div style={{fontWeight:800,fontSize:11,color:g.color,marginBottom:6,letterSpacing:.3}}>
                          {g.label} ({grouped[g.key].length})
                        </div>
                        <div style={{display:"flex",gap:5,flexWrap:"wrap"}}>
                          {grouped[g.key].map(it=>(
                            <span key={it.kode} style={{background:"#fff",border:`1px solid ${g.border}`,
                              borderRadius:6,padding:"3px 9px",fontSize:11,fontWeight:600,color:"#475569"}}>
                              {it.nama}
                            </span>
                          ))}
                        </div>
      ...
      {assignModal&&(()=>{
        ...
        return(
          <Modal title={`${existing?"Edit":"Distribusi"} Pekerja — ${task.proses}`} onClose={()=>{setAssignModal(null);setSelPekerja([]);}} width={460}>
            ...
            {pekerjaDivisi.length===0?(
              <div style={{padding:"16px",background:"#f8fafc",borderRadius:8,fontSize:12,color:"#94a3b8",textAlign:"center"}}>
                Belum ada pekerja di divisi {dc.label}. Tambahkan di tab Master Pekerja.
              </div>
            ):(
              <div style={{display:"flex",flexDirection:"column",gap:6,marginBottom:16}}>
                {pekerjaDivisi.map(p=>{
...
function OperatorView({woData,setWoData,user,renhar,setRenhar,pekerja,setKendalaLog}){
  ...
  const myProses=cfg.proses||[];

  // PIC options: all users of same divisi (kept for display in header only)
  const picLabel=user.name;

  // Tasks for today grouped by proses
  const todayTasks=useMemo(()=>
    renhar.filter(r=>r.divisi===user.divisi&&r.tanggal===viewDate)
  ,[renhar,user.divisi,viewDate]);

  const nextDate=useMemo(()=>{
    if(todayTasks.length)return null;
    const f=renhar.filter(r=>r.divisi===user.divisi&&r.tanggal>viewDate).map(r=>r.tanggal).sort();
    return f[0]||null;
  ...
  const lockProgress=()=>{
    ...
    // hitung dulu apakah ada yang bisa dikunci (sebelum setWoData)
    let hasChanges=false;
    todayTasks.forEach(task=>{
      const panel=woData.flatMap(w=>w.panels).find(p=>p.id===task.panelId);
      if(!panel)return;
      const processed=new Set();
      (task.komponen||[]).forEach(kode=>{
        ...
        myProses.forEach(pr=>{
          ...
          if(pct===0)return;
          newLocked[`${panel.id}_${kode}_${pr}_${viewDate}_${shift}`]=true;
          hasChanges=true;
        });
      });
    });
    ...
    // update history di woData
    setWoData(prev=>prev.map(w=>({...w,panels:w.panels.map(p=>{
      const relatedTasks=todayTasks.filter(t=>t.panelId===p.id);
      if(!relatedTasks.length)return p;
      ...
      relatedTasks.forEach(task=>{
        (task.komponen||[]).forEach(kode=>{
          ...
          myProses.forEach(pr=>{
            ...
            const existIdx=prevHist.findIndex(h=>h.tanggal===viewDate&&String(h.shift)===String(shift));
            if(existIdx>=0){
              if(prevHist[existIdx].pct!==pct){
                prevHist[existIdx]={...prevHist[existIdx],pct,ts:new Date().toISOString()};
                newChecklist[kode]={...cl,history:{...(cl.history||{}),[pr]:prevHist}};
              }
            } else {
              prevHist.push({pct,tanggal:viewDate,shift,ts:new Date().toISOString()});
              newChecklist[kode]={...cl,history:{...(cl.history||{}),[pr]:prevHist}};
            }
          });
        });
  ...
  const carryOverUnfinished=(updatedWoData)=>{
    ...
    setRenhar(prev=>{
      let updated=[...prev];
      todayTasks.forEach(task=>{
        const panel=updatedWoData.flatMap(w=>w.panels).find(p=>p.id===task.panelId);
        if(!panel)return;
        // komponen yg belum 100% di semua proses milik divisi ini
        const unfinishedKomp=(task.komponen||[]).filter(kode=>{
          const cl=panel.checklist[kode]||{};
          const divisiProses=DIVISI_CONFIG[user.divisi]?.proses||[];
          return divisiProses.some(pr=>getLatestProgress(cl,pr)<100);
        });
        if(unfinishedKomp.length===0)return;
        // cek apakah sudah ada entry di hari berikutnya untuk task ini
        const existIdx=updated.findIndex(r=>
        ...
        if(existIdx>=0){
          ...
          updated=[...updated,{
            ...
            id:uid(),
            tanggal:nextDate,
            komponen:unfinishedKomp,
            carryOver:true,
            pekerja:task.pekerja||[],
          }];
        }
      });
      return updated;
  ...
  // Cek apakah cell terkunci untuk tanggal+shift HARI INI
  const isCellLocked=(panelId,kode,proses)=>
    !!lockedCells[`${panelId}_${kode}_${proses}_${viewDate}_${shift}`];

  // Ambil nilai qty terkunci tertinggi dari semua hari sebelumnya
  const getLockedFloor=(panelId,kode,proses)=>{
    const panel=woData.flatMap(w=>w.panels).find(p=>p.id===panelId);
    const hist=panel?.checklist[kode]?.history?.[proses]||[];
    if(!hist.length)return 0;
    // ambil qty terkunci tertinggi dari qtyProsesByDate semua tanggal sebelum viewDate
    const qtyByDate=panel?.checklist[kode]?.qtyProsesByDate?.[proses]||{};
    const pastDates=Object.keys(qtyByDate).filter(d=>d<viewDate);
    if(!pastDates.length)return 0;
    return Math.max(...pastDates.map(d=>qtyByDate[d]||0));
  };

  const updateQtyProses=(panelId,kode,proses,val)=>{
    if(isCellLocked(panelId,kode,proses))return;
    const floor=getLockedFloor(panelId,kode,proses);
    setWoData(prev=>{
      const updated=prev.map(w=>({...w,panels:w.panels.map(p=>{
        if(p.id!==panelId)return p;
        const cl=p.checklist[kode];
      ...
      setTimeout(()=>carryOverUnfinished(updated),0);
      return updated;
    });
  };

  const updatePctManual=(panelId,kode,proses,pct)=>{
    setWoData(prev=>{
      const updated=prev.map(w=>({...w,panels:w.panels.map(p=>{
        if(p.id!==panelId)return p;
        const cl=p.checklist[kode];
    ...
    return(
      <div style={{padding:20,maxWidth:480,margin:"0 auto"}} className="fi">
        ...
        <Card>
          <div style={{marginBottom:16}}>
            <Lbl>Tanggal</Lbl>
            <Inp type="date" value={viewDate} onChange={e=>setViewDate(e.target.value)}/>
          </div>
          <div style={{marginBottom:16}}>
            <Lbl>Shift</Lbl>
            <div style={{display:"flex",gap:10}}>
    ...
    return(
      <div style={{padding:16,maxWidth:520,margin:"0 auto"}} className="fi">
        ...
        <div style={{textAlign:"center",padding:"40px 20px"}}>
          ...
          {nextDate&&(
            <div style={{background:"#eff6ff",border:"1px solid #bfdbfe",borderRadius:12,padding:"14px 20px",display:"inline-block"}}>
              <div style={{fontSize:12,color:"#2563eb",fontWeight:600}}>Jadwal terdekat:</div>
              <div style={{fontSize:14,fontWeight:700,color:"#1d4ed8",marginTop:2}}>{fmtDate(nextDate)}</div>
              <button onClick={()=>setViewDate(nextDate)} style={{marginTop:8,padding:"6px 18px",borderRadius:7,border:"none",cursor:"pointer",background:"#2563eb",color:"#fff",fontWeight:700,fontSize:12}}>Lihat Jadwal</button>
            </div>
          )}
        </div>
      </div>
  ...
  return(
    <div style={{padding:16}} className="fi">
      ...
      <div style={{display:"flex",gap:8,marginBottom:16,flexWrap:"wrap"}}>
        {myProses.map(pr=>{
          const tasksForProses=tasksByProses[pr]||[];
          if(!tasksForProses.length)return null;
          const totalKomp=tasksForProses.reduce((a,t)=>{
            const p=woData.flatMap(w=>w.panels).find(x=>x.id===t.panelId);
            return a+(t.komponen||[]).filter(k=>(p?.checklist[k]?.qty||0)>0).length;
          },0);
          const doneKomp=tasksForProses.reduce((a,t)=>{
            const p=woData.flatMap(w=>w.panels).find(x=>x.id===t.panelId);
            return a+(t.komponen||[]).filter(k=>getLatestProgress(p?.checklist[k],pr)===100).length;
          },0);
          const pc=PROSES_COLOR[pr]||"#64748b";
          return(
          ...
          {proyekList.length>1&&(
            <select value={fProyek} onChange={e=>{setFProyek(e.target.value);setFPanel("ALL");}}
              style={{padding:"4px 10px",borderRadius:8,border:"1.5px solid #e2e8f0",
                background:"#f8fafc",fontSize:11,fontWeight:600,color:"#475569",cursor:"pointer"}}>
              <option value="ALL">Semua Proyek</option>
              {proyekList.map(p=><option key={p} value={p}>{p}</option>)}
            </select>
          )}
          {panelList.length>1&&(
            <select value={fPanel} onChange={e=>setFPanel(e.target.value)}
              ...
              <option value="ALL">Semua Panel</option>
              {panelList.map(p=><option key={p} value={p}>{p}</option>)}
            </select>
          )}
      ...
      {myProses.map(proses=>{
        ...
        // Build rows: per komponen per task
        const rows=[];
        tasks.forEach(task=>{
          const panel=woData.flatMap(w=>w.panels).find(p=>p.id===task.panelId);
          const wo=panel?woData.find(w=>w.panels.some(p=>p.id===task.panelId)):null;
          const panelCfg=panel?PANEL_TYPES[panel.tipe]:null;
            ...
            <div style={{background:pc,padding:"10px 16px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
              ...
              <div style={{display:"flex",gap:8,alignItems:"center"}}>
                ...
                <span style={{background:"#ffffff22",color:"#fff",borderRadius:20,padding:"2px 10px",fontSize:11,fontWeight:700}}>
                  {rows.filter(r=>isDone(r)).length}/{rows.length} selesai
                </span>
              </div>
            </div>
              ...
              <table style={{width:"100%",borderCollapse:"collapse",fontSize:11}}>
                ...
                <tbody>
                  {rows.map((r,ri)=>{
                    ...
                    const td={padding:"6px 8px",borderBottom:"1px solid #f1f5f9",borderRight:"1px solid #f1f5f9",
                      background:rBg,verticalAlign:"middle"};
                    const wpDef=PANEL_TYPES[r.panel.tipe]?.wps.find(w=>w.items.some(it=>it.kode===r.kode));
                    return(
                      <tr key={`${r.task.id}-${r.kode}`}>
                        ...
                        <td style={{...td,textAlign:"center"}}>
                          {wpDef&&<span style={{background:wpDef.color+"18",color:wpDef.color,border:`1px solid ${wpDef.color}33`,borderRadius:4,padding:"1px 6px",fontSize:10,fontWeight:700}}>{wpDef.wp}</span>}
                        </td>
                        ...
                        {isQtyBased&&(()=>{
                          const locked=isCellLocked(r.panel.id,r.kode,proses);
                          const floor=getLockedFloor(r.panel.id,r.kode,proses);
                          return(
                            <td style={{...td,textAlign:"center"}}>
                              {locked?(
                                /* Terkunci hari ini */
                                <div style={{display:"flex",alignItems:"center",gap:4,justifyContent:"center"}}>
                                  <span style={{width:60,padding:"4px 6px",borderRadius:7,
                                ...
                                /* Bisa input, tapi min = floor dari kunci sebelumnya */
                                <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:2}}>
                                  <input type="number" min={floor} max={r.qtyKomp} value={r.qtyProses}
                                    onChange={e=>updateQtyProses(r.panel.id,r.kode,proses,e.target.value)}
                          ...
                          return(
                            <td key={s} style={{...td,textAlign:"center",padding:"4px",
                              ...
                              <button
                                onClick={()=>{
                                  if(reached){
                                    // uncheck: kembali ke step sebelumnya
                                    updatePctManual(r.panel.id,r.kode,proses,prevStep);
                                  } else {
                                    updatePctManual(r.panel.id,r.kode,proses,s);
                                  }
                                }}
      ...
      {/* TOMBOL KUNCI PROGRESS HARI INI */}
      {todayTasks.length>0&&(
...
if(page==="landing") return <LandingPage onEnter={()=>setPage("login")}/>;
  if(!user)return <Login onLogin={u=>{
    setUser(u);
    setPage("app");
    setTab(OPERATOR_ROLES.includes(u.divisi)?"operator":"dashboard");
  }}/>;
