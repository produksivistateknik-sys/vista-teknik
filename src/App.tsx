...
﻿function RecycleBinTab({user}:any){
  ...
  const restore=async(tbl:string,id:number,setter:any)=>{
    await supabase.from(tbl).update({deleted_at:null,deleted_by:null}).eq("id",id);
    setter((p:any[])=>p.filter(x=>x.id!==id));
  };
  const permDel=async(tbl:string,id:number,setter:any)=>{
    await supabase.from(tbl).delete().eq("id",id);
    setter((p:any[])=>p.filter(x=>x.id!==id));
  };
  const fmt=(ts:string)=>{if(!ts)return"—";return new Date(ts).toLocaleDateString("id-ID",{day:"numeric",month:"short",year:"numeric"})+" "+new Date(ts).toLocaleTimeString("id-ID",{hour:"2-digit",minute:"2-digit"});};
  ...
  const Empty=({label}:any)=>(<div style={{textAlign:"center",padding:"60px",color:"#94a3b8"}}><div style={{fontSize:32,marginBottom:8}}>🗑</div><div style={{fontSize:13,fontWeight:600}}>Recycle bin kosong — tidak ada {label} yang dihapus</div></div>);
  const renderTable=(data:any[],setter:any,tbl:string,cols:any[])=>(
    data.length===0?<Empty label={tbl==="mesin"?"mesin":"work order"}/>:
    <div style={{overflowX:"auto",borderRadius:10,border:"1px solid #e2e8f0"}}>
      <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
        <thead><tr>{cols.map((c:any)=><th key={c.key} style={{...thS,textAlign:c.center?"center":"left"}}>{c.label}</th>)}<th style={{...thS,textAlign:"center"}}>Aksi</th></tr></thead>
        <tbody>{data.map((row:any,i:number)=>{
          const bg=i%2===0?"#fff":"#f8fafc";
          const td:any={padding:"9px 10px",borderBottom:"1px solid #f1f5f9",borderRight:"1px solid #f1f5f9",background:bg,verticalAlign:"middle",opacity:0.8};
          return(<tr key={row.id}>
            {cols.map((c:any)=><td key={c.key} style={{...td,textDecoration:c.strike?"line-through":"none",color:c.strike?"#94a3b8":"#475569",fontFamily:c.mono?"monospace":"inherit"}}>{c.fmt?c.fmt(row[c.key]):row[c.key]||"—"}</td>)}
            <td style={{...td,textAlign:"center"}}>
              <div style={{display:"flex",gap:5,justifyContent:"center"}}>
                <button onClick={()=>restore(tbl,row.id,setter)} style={{background:"#f0fdf4",border:"1px solid #bbf7d0",borderRadius:6,padding:"4px 10px",cursor:"pointer",fontSize:11,color:"#16a34a",fontWeight:700}}>↩ Pulihkan</button>
                <button onClick={()=>permDel(tbl,row.id,setter)} style={{background:"#fef2f2",border:"1px solid #fecaca",borderRadius:6,padding:"4px 8px",cursor:"pointer",fontSize:11,color:"#dc2626"}}>🗑 Permanen</button>
              </div>
            </td>
          </tr>);
        })}</tbody>
    ...
    <div>
      <div style={{display:"flex",gap:8,marginBottom:14,alignItems:"center"}}>
        <div style={{display:"flex",gap:0,border:"1px solid #e2e8f0",borderRadius:8,overflow:"hidden"}}>
          <button onClick={()=>setActiveSection("mesin")} style={{padding:"6px 16px",border:"none",cursor:"pointer",fontSize:12,fontWeight:700,background:activeSection==="mesin"?"#1d4ed8":"#fff",color:activeSection==="mesin"?"#fff":"#64748b",borderRight:"1px solid #e2e8f0"}}>⚙️ Mesin ({deletedMesin.length})</button>
          <button onClick={()=>setActiveSection("wo")} style={{padding:"6px 16px",border:"none",cursor:"pointer",fontSize:12,fontWeight:700,background:activeSection==="wo"?"#1d4ed8":"#fff",color:activeSection==="wo"?"#fff":"#64748b"}}>📋 Work Order ({deletedWO.length})</button>
        </div>
      ...
      {loading?<div style={{textAlign:"center",padding:"40px",color:"#94a3b8"}}>Memuat...</div>:
        activeSection==="mesin"?renderTable(deletedMesin,setDeletedMesin,"mesin",[
          {key:"kode",label:"Kode",mono:true,strike:true},
          {key:"nama",label:"Nama Mesin",strike:true},
          {key:"lokasi",label:"Lokasi"},
          {key:"deleted_at",label:"Dihapus Pada",fmt:fmt},
          {key:"deleted_by",label:"Dihapus Oleh"},
        ]):renderTable(deletedWO,setDeletedWO,"work_orders",[
          {key:"wo",label:"No WO",mono:true,strike:true,fmt:(v:any)=>"WO-"+v},
          {key:"proyek",label:"Proyek",strike:true},
  ...
  return(
    <div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,marginBottom:14}}>
        {stats.map((s:any,i:number)=>(<Card key={i} style={{padding:"12px 16px",borderLeft:`3px solid ${s.c}`}}><div style={{fontSize:22,fontWeight:800,color:s.c}}>{s.v}</div><div style={{fontSize:10,color:"#94a3b8",fontWeight:600,textTransform:"uppercase" as const,letterSpacing:.3,marginTop:2}}>{s.l}</div></Card>))}
      </div>
      ...
      {showForm&&(
        <Card style={{marginBottom:14,border:"2px solid #2563eb"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:12}}>
            <div><Lbl>Kendala</Lbl><textarea value={form.kendala} onChange={e=>setForm({...form,kendala:e.target.value})} placeholder="Deskripsi kendala..." style={{width:"100%",padding:"9px 12px",borderRadius:8,border:"1.5px solid #e2e8f0",background:"#f8fafc",color:"#1e293b",fontSize:12,resize:"vertical" as const,minHeight:72,fontFamily:"inherit"}}/></div>
            <div><Lbl>Perbaikan</Lbl><textarea value={form.perbaikan} onChange={e=>setForm({...form,perbaikan:e.target.value})} placeholder="Tindakan perbaikan..." style={{width:"100%",padding:"9px 12px",borderRadius:8,border:"1.5px solid #e2e8f0",background:"#f8fafc",color:"#1e293b",fontSize:12,resize:"vertical" as const,minHeight:72,fontFamily:"inherit"}}/></div>
          </div>
...
function MaintenanceRutinTab({mesinList,rutinList,setRutinList,user,today,terlambat,mingguIni}:any){
  ...
  return(
    <div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,marginBottom:14}}>
        {[{l:"Terlambat",v:terlambat.length,c:"#dc2626"},{l:"Jatuh tempo minggu ini",v:mingguIni.length,c:"#f59e0b"},{l:"Total jadwal",v:rutinList.length,c:"#2563eb"},{l:"Kepatuhan",v:kepatuhan+"%",c:"#16a34a"}].map((s:any,i:number)=>(<Card key={i} style={{padding:"12px 16px",borderLeft:`3px solid ${s.c}`}}><div style={{fontSize:22,fontWeight:800,color:s.c}}>{s.v}</div><div style={{fontSize:10,color:"#94a3b8",fontWeight:600,textTransform:"uppercase" as const,letterSpacing:.3,marginTop:2}}>{s.l}</div></Card>))}
      </div>
      {([...terlambat,...mingguIni]).length>0&&(
        <div style={{marginBottom:14}}>
          <div style={{fontSize:12,fontWeight:700,color:"#64748b",textTransform:"uppercase" as const,letterSpacing:.4,marginBottom:8}}>⚠️ Perlu perhatian</div>
          <div style={{display:"flex",flexDirection:"column" as const,gap:6}}>
            {[...terlambat,...mingguIni].slice(0,4).map((r:any)=>{const st=getStatus(r);const fc=FC[r.frekuensi]||FC.bulanan;return(
              <div key={r.id} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",background:st.bg,border:`1px solid ${st.color}30`,borderRadius:8,borderLeft:`3px solid ${st.color}`}}>
                ...
                <span style={{fontSize:11,fontWeight:700,color:st.color,whiteSpace:"nowrap" as const}}>{st.label}</span>
      ...
      {showForm&&(
        <Card style={{marginBottom:14,border:"2px solid #2563eb"}}>
          <div style={{fontWeight:800,fontSize:14,color:"#1e293b",marginBottom:12}}>{editId?"✏️ Edit":"➕ Tambah Jadwal Rutin"}</div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12,marginBottom:12}}>
              ...
              <tr key={r.id}>
                ...
                <td style={{...td,textAlign:"center" as const}}>
                  <div style={{display:"flex",gap:4,justifyContent:"center"}}>
                    <button onClick={()=>setDoneId(r)} style={{background:"#f0fdf4",border:"1px solid #bbf7d0",borderRadius:6,padding:"3px 7px",cursor:"pointer",fontSize:11,color:"#16a34a",fontWeight:700}}>✓</button>
...
function RecycleBinTab({user}:any){
  ...
  const CATS:any={
    ...
    mesin:{label:"Mesin"},
    pekerja:{label:"Pekerja"},
    raw_schedule:{label:"Raw Schedule"},
    renhar:{label:"Rencana Harian"},
    kendala:{label:"Kendala"},
  };
  useEffect(()=>{
    const load=async()=>{
      setLoading(true);
