...
function RencanaHarian({rawData,woData,renhar,setRenhar,pekerja,createRenhar,updateRenhar,removeRenhar}){
    ...
    rawData.forEach(row=>{
      ...
      entries.forEach(e=>{
        tasks.push({
          ...
          panelId:row.panel_id??row.panelId,
          proyek:row.proyek,
          panel:row.panel,
          proses:row.proses,
  ...
  const byProses=useMemo(()=>{
    const map={};
    ALL_PROSES.forEach(p=>{if(filteredTasks.some(t=>t.proses===p))map[p]=filteredTasks.filter(t=>t.proses===p);});
    return map;
  },[filteredTasks]);

  const prosesList=ALL_PROSES.filter(p=>byProses[p]);
  ...
  const getRenharEntry=(task)=>renhar.find(r=>
    (r.raw_id||r.rawId)===task.rawId &&
    r.wp===task.wp &&
    r.tanggal===task.tanggal
  );
