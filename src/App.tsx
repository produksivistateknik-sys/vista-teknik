...
function KapasitasPekerjaanTab(){
  ...
  const exportProcessTimeExcel=()=>{
    ...
    const rows:any[]=[["KODE","NAMA KOMPONEN","TIPE PANEL","WP","JENIS PEKERJAAN","MENIT/PCS"]];
    processList.forEach((p:any)=>{
      rows.push([p.kode_komponen,p.nama_komponen,p.tipe_panel,p.wp,p.jenis_pekerjaan,p.menit_per_pcs]);
    });
    const ws=XLSX.utils.aoa_to_sheet(rows);
    XLSX.utils.book_append_sheet(wb,ws,"Process Time");
    ...
    reader.onload=async(evt:any)=>{
      try{
        ...
        for(let i=1;i<rows.length;i++){
          ...
          const wp=String(row[3]||"").trim();
          const jenis=String(row[4]||"").trim();
          const menit=Number(row[5])||0;
          if(!kode||!tipe||!jenis)continue;
