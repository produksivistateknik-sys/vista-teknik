      ...
      <div className="fi">
          ...
          {foto.length>0?(
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(120px,1fr))",gap:8}}>
              {foto.map((f:any,fi:number)=>(
                <div key={fi} onClick={()=>setLightbox({...f,_label:`${tugas.label}_${selectedPanel.nama}`})} style={{cursor:"pointer"}} className="kp-foto-print">
        ...
        {lightbox&&(
          <div onClick={()=>setLightbox(null)} className="no-print"
            ...
            <button onClick={(e:any)=>{e.stopPropagation();downloadFotoTunggal(lightbox.url,sanitizeNamaFile(`${lightbox._label||"foto"}.jpg`))}}
