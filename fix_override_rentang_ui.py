...
with open(file_path + ".bak_override_rentang_ui", "w", encoding="utf-8", errors="replace") as f:
    f.write(content)
print("[OK] Backup dibuat")

# Fix: Tambah toggle mode SEBELUM form yang ada, dan bungkus form lama dengan kondisi single
OLD_FORM_START = '''          <div style={{background:"#f0f8ff",borderRadius:10,border:"1.5px solid #bfdbfe",padding:"14px 16px",marginBottom:16}}>
            <div style={{fontWeight:700,fontSize:13,color:"#1e293b",marginBottom:12}}>{editOverride?"\u270f\ufe0f Edit Override":"\u2795 Tambah Overrride"}</div>'''

NEW_FORM_START = '''          <div style={{display:"flex",gap:2,marginBottom:14,background:"#f1f5f9",borderRadius:8,padding:3,width:"fit-content"}}>
...
ok = OLD_FORM_START in content
print(f"  PATTERN: {'FOUND' if ok else 'MISSING'}")

if ok:
    content = content.replace(OLD_FORM_START, NEW_FORM_START)
    # Tutup div tambahan overrideMode==="single" - cari closing form lama, tambah )}
    OLD_FORM_END = '''              </div>
            </div>
          </div>
    ...
    NEW_FORM_END = '''              </div>
          ...
          <div style={{overflowX:"auto" as const,borderRadius:10,border:"1px solid #e2e8f0"}}>
            <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>'''
    ok2 = OLD_FORM_END in content
    print(f"  FORM_END: {'FOUND' if ok2 else 'MISSING'}")
    if ok2:
        content = content.replace(OLD_FORM_END, NEW_FORM_END, 1)
        with open(file_path, "w", encoding="utf-8", errors="replace") as f:
            f.write(content)
        print("[OK] UI toggle mode + form rentang tanggal berhasil ditambah")
        print("[INFO] Jalankan: npm run build")
    else:
        print("[FAIL] FORM_END tidak ketemu, TIDAK menyimpan apapun")
